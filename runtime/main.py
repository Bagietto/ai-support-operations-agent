...
def _salvar_json_com_fallback(caminho: Path, dados: dict) -> Path:
    """Salva JSON e usa arquivo alternativo se o destino estiver bloqueado."""
    ...
    try:
        ...
        alternativo = caminho.with_name(f"{caminho.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}{caminho.suffix}")
        try:
            alternativo.write_text(conteudo, encoding="utf-8")
            print(f"  Aviso: sem permissao para sobrescrever {caminho}; salvo em {alternativo}")
            return alternativo
        except PermissionError:
            fallback_dir = Path.cwd() / "resultados"
            fallback_dir.mkdir(parents=True, exist_ok=True)
            fallback = fallback_dir / alternativo.name
            fallback.write_text(conteudo, encoding="utf-8")
            print(f"  Aviso: sem permissao em {caminho.parent}; salvo em {fallback}")
            return fallback
...
def main():
    ...
    parser_comparar.add_argument("--incluir-padrao", action="store_true", help="Inclui baseline sem arquitetura alternativa")
    ...
    parser_tool_cmp.add_argument("--incluir-padrao", action="store_true", help="Inclui baseline sem arquitetura alternativa")
    ...
    if argumentos.comando == "rodar":
        ...
        caminho_salvo = _salvar_json_com_fallback(caminho_resultado, resultado)
        print(f"  Resultado salvo: {caminho_salvo}")
        ...
        arquiteturas = ["react", "plan_execute", "reflect"]
        if getattr(argumentos, "incluir_padrao", False):
            arquiteturas.insert(0, "padrao")
        todos_resultados = []
        for arq in arquiteturas:
            ...
            _salvar_json_com_fallback(caminho_res, resultado)
        ...
        caminho_salvo = _salvar_json_com_fallback(caminho_res, resultado)
        print(f"  Resultado salvo: {caminho_salvo}")
