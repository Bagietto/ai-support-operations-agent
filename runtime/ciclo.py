...
def _ultimo_dado_ferramenta(estado: dict, nome_ferramenta: str) -> dict:
    for etapa in reversed(estado.get("historico", [])):
        plano = etapa.get("plano", {})
        if plano.get("nome_ferramenta") != nome_ferramenta:
            continue
        resultado = etapa.get("resultado_acao") or {}
        dados = resultado.get("dados") or {}
        return dados if isinstance(dados, dict) else {}
    return {}


def _resolver_caminho_banco_suporte() -> Path:
    conn_string = os.environ.get("DB_CONNECTION_STRING") or str(Path(__file__).resolve().parent.parent / "dados" / "suporte.db")
    caminho = Path(conn_string)
    return caminho


def _gravar_triagem_sqlite(caminho_db: Path, valores: tuple) -> None:
    import sqlite3
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS triagens_pendentes (
            ...
            fila_recomendada TEXT NOT NULL,
            time_recomendado TEXT NOT NULL,
            decisao_final TEXT NOT NULL,
            acoes_sugeridas TEXT NOT NULL,
    ...
    conn.execute(
        """
        INSERT INTO triagens_pendentes (
            ticket_id, trace_id, entrada, categoria, subcategoria, prioridade,
            confidence_score, motivo, fila_recomendada, time_recomendado,
            decisao_final, acoes_sugeridas, criado_em
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(ticket_id) DO UPDATE SET
            ...
            fila_recomendada=excluded.fila_recomendada,
            time_recomendado=excluded.time_recomendado,
            decisao_final=excluded.decisao_final,
            acoes_sugeridas=excluded.acoes_sugeridas,
...
def _persistir_triagem_pendente(estado: dict, trace_id: str) -> None:
    veredito = _ultimo_dado_ferramenta(estado, "gerar_veredito_suporte")
    roteamento = _ultimo_dado_ferramenta(estado, "sugerir_roteamento")
    classificacao = _ultimo_dado_ferramenta(estado, "classificar_ticket")
    prioridade = _ultimo_dado_ferramenta(estado, "calcular_prioridade")

    decisao = str(veredito.get("decisao_final") or "")
    precisa_humano = bool(veredito.get("necessita_aprovacao_humana"))
    if decisao != "human_intervention_required" and not precisa_humano:
        return

    ticket_id = (
        classificacao.get("_entrada", {}).get("ticket_id")
        or veredito.get("_entrada", {}).get("ticket_id")
        or "SUP-0000"
    )
    motivo = veredito.get("comentario_interno") or veredito.get("justificativa_confidence") or "requer intervencao humana"
    valores = (
        ticket_id,
        trace_id,
        estado.get("entrada", ""),
        classificacao.get("categoria", "indefinida"),
        classificacao.get("subcategoria", "indefinida"),
        prioridade.get("prioridade", ""),
        float(veredito.get("confidence_score") or 0.0),
        motivo,
        roteamento.get("fila_recomendada", "support.triage"),
        roteamento.get("time_recomendado", "triage"),
        decisao or "human_intervention_required",
        json.dumps(veredito.get("acoes_sugeridas", []), ensure_ascii=False),
        datetime.now().astimezone().isoformat(timespec="seconds"),
    )

    candidatos = [_resolver_caminho_banco_suporte()]
    fallback = Path("operacional.db")
    if fallback not in candidatos:
        candidatos.append(fallback)

    ultimo_erro = None
    for caminho_db in candidatos:
        try:
            _gravar_triagem_sqlite(caminho_db, valores)
            print(f"  [triagem] pendencia registrada no SQLite: {caminho_db} ({ticket_id})")
            return
        except Exception as erro:
            ultimo_erro = erro
    print(f"  [triagem] erro ao registrar pendencia: {ultimo_erro}")
...
def rodar(caminho_agente: str, texto_entrada: str, modo: str = None, evento: str = None, saida: str = None, arquitetura: str = None) -> dict:
    ...
    while not estado["concluido"] and estado["etapa"] < estado["max_etapas"]:
                ...
                if ferramenta_fallback:
                    ...
                    plano = {
                        ...
                        "argumentos_ferramenta": montar_argumentos_mock(
                            habilidade_fb,
                            estado["historico"],
                            entrada_texto=estado.get("entrada", ""),
                        ),
        ...
        if plano.get("proxima_acao") == "CHAMAR_FERRAMENTA" and plano.get("nome_ferramenta"):
            ...
            proxima_obrigatoria = faltantes[0] if faltantes else None
            if proxima_obrigatoria and plano.get("nome_ferramenta") != proxima_obrigatoria:
                habilidades = contratos.get("habilidades", {}).get("habilidades", [])
                habilidade_faltante = next(
                    (hab for hab in habilidades if hab.get("nome") == proxima_obrigatoria),
                    {},
                )
                print(
                    f"  [regras] sequencia obrigatoria: {plano.get('nome_ferramenta')} "
                    f"adiado; redirecionando para {proxima_obrigatoria}"
                )
                plano = {
                    "proxima_acao": "CHAMAR_FERRAMENTA",
                    "nome_ferramenta": proxima_obrigatoria,
                    "argumentos_ferramenta": montar_argumentos_mock(
                        habilidade_faltante,
                        estado["historico"],
                        entrada_texto=estado.get("entrada", ""),
                    ),
                    "criterio_sucesso": f"{proxima_obrigatoria} obrigatorio na sequencia do pipeline",
                }
        ...
        if plano.get("proxima_acao") == "FINALIZAR" and contrato_critico:
            ...
            if reflexoes_feitas < max_reflexoes:
                ...
                if aprovado or nota >= limiar:
                    ...
                    # Redirecionar apenas para uma ferramenta sugerida que exista e ainda possa ser chamada.
                    # Se a critica nao produzir uma acao executavel, mantemos FINALIZAR para evitar
                    # encerrar por limite ao repetir uma ferramenta ja consumida.
                    ...
                    if sugestoes:
                        ...
                        for sug in sugestoes:
                            for hn in habilidades_nomes:
                                if hn in str(sug):
                                    chamadas_feitas = estado["chamadas_por_ferramenta"].get(hn, 0)
                                    limite_ferramenta = estado["limites_por_ferramenta"].get(hn)
                                    if not limite_ferramenta or chamadas_feitas < limite_ferramenta:
                    ...
                    if ferramenta_correcao:
                        ...
                        plano["criterio_sucesso"] = (
                            "finalizar apos reflexao sem ferramenta de correcao executavel; "
                            f"problemas apontados: {'; '.join(problemas_critica[:2])}"
                        )
                        print("  [reflexao] nenhuma ferramenta de correcao executavel; finalizando com ressalvas.")
            else:
    ...
    _persistir_triagem_pendente(estado, tel.trace_id)
